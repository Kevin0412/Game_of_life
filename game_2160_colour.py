...
video = cv2.VideoWriter("game_colour.mp4", cv2.VideoWriter_fourcc('I', '4', '2', '0'), 8,(2160,2160))
'''h=open("game.csv")
H=h.read()
Hh=H.split("\n")
data=[]
        ...
        if 2 in data[x] or 3 in data[x]:
            for y in range(1077-n,1083+n):
                if int(data[x][y])==1 or int(data[x][y])==2:
                    if int(data[x][y])==2:
                        if int((n%765)/255)==0:
                            img[x,y]=(255-n%255,n%255,255)
                        if int((n%765)/255)==1:
                            img[x,y]=(n%255,255,255-n%255)
                        if int((n%765)/255)==2:
                            img[x,y]=(255,255-n%255,n%255)
                    data[x][y]=1
                else:
                    if int(data[x][y])==3:
                        if int((n%765)/255)==0:
                            img[x,y]=(n%255,255-n%255,0)
                        if int((n%765)/255)==1:
                            img[x,y]=(255-n%255,0,n%255)
                        if int((n%765)/255)==2:
                            img[x,y]=(0,n%255,255-n%255)
                    data[x][y]=0
    for x in range(1077-n,1083+n):
        for y in range(1077-n,1083+n):
            develop(x,y)
...
for g in range(1,765):
    ...
    if cv2.waitKey(1)&0xFF==27:
        break
cv2.imwrite("game_colour.bmp",img)
video.release()
